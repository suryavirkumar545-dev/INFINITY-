...
selected_photo = get_random_start_photo(message.from_user.id)
...
try:
    ...
    # Telegram flood limit आने पर wait करके दुबारा भेजें
    ...
    try:
        ...
        LOGGER.exception(
            "Start photo retry failed for user %s: %s",
            message.from_user.id,
            retry_error,
        )

        # Photo fail होने पर भी /start response जरूर मिलेगा
    ...
    LOGGER.exception(
        "Start photo failed for user %s using %s: %s",
        message.from_user.id,
        selected_photo,
        error,
    )

    # Broken URL/Telegram error होने पर text fallback
    await message.reply_text(
        caption,
        reply_markup=reply_markup,
        disable_web_page_preview=True,
